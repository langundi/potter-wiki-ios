//
//  PotionDetailViewController.swift.swift
//  PotterWiki
//
...
class PotionDetailViewController: UIViewController {
    ...
    private var potionImage: UIImageView = {
        let iv = UIImageView()
    ...
    // MARK: - Lifecycle
    init(potion: PotionModel) {
        self.potion = potion
        super.init(nibName: nil, bundle: nil)
        
        if let url = potion.attributes.image {
            let imageURL = URL(string: url)
            potionImage.kf.setImage(with: imageURL, options: [.transition(.fade(0.2)), .cacheOriginalImage])
        } else {
            potionImage.image = .none
        }
        
        name.text = potion.attributes.name
        
        informationSection.configure(attributes: potion.attributes)
    }
    ...
    private func setupUI() {
        ...
        headerVStack.addArrangedSubview(potionImage)
        headerVStack.addArrangedSubview(name)
        
        if let link = potion.attributes.wiki {
            headerVStack.addArrangedSubview(wikiLink)
            configure(link: link, displayText: "Fandom")
        }
        
        NSLayoutConstraint.activate([
            potionImage.widthAnchor.constraint(equalToConstant: 180),
