...
class AffiliationSectionView: UIView {
    ...
    private let occupation = AttributesView(title: "Occupation:")
    private let house = AttributesView(title: "House:")
    ...
    init() {
        ...
        contentStack.addArrangedSubview(occupation)
        contentStack.addArrangedSubview(house)
    ...
    func configure(attributes: CharacterAttributes) {
        occupation.configure(attributes: attributes.jobs ?? [])
        house.configure(attributes: [attributes.house ?? "-"])
    }
}
